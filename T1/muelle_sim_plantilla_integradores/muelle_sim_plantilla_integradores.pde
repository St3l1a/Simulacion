...
IntegratorType integrator = IntegratorType.RK2;  // ODE integration method

/////////////////////////////////////////////////////////////////////
// Draw values:
final boolean FULL_SCREEN = false;
final int     DRAW_FREQ = 60;   // Draw frequency (Hz or Frame-per-second)
int           DISPLAY_SIZE_X = 600;   // Display width (pixels)
int           DISPLAY_SIZE_Y = 400;   // Display height (pixels)
...
void updateSimulationRK4()
{
  ...
  // k1s = v(t)*h
  _a = calculateAcceleration(_s,_v);
  PVector k1v = PVector.add(_v,PVector.mult(_a,SIM_STEP));
  PVector k1s = PVector.add(_v,PVector.mult(_v,SIM_STEP));
  
  // k2v = a(s(t)+k1s/2, v(t)+k1v/2)*h
  // k2s = (v(t)+k1v/2)*h
  PVector v2 = PVector.add(_v, PVector.mult(k1v,0.5));
  PVector a2 = calculateAcceleration(PVector.add(_v, PVector.mult(k1s,0.5)),v2);
  PVector k2v = PVector.mult(a2,SIM_STEP);
  ...
  PVector a3 = calculateAcceleration(PVector.add(_v, PVector.mult(k2s,0.5)),v3);
  PVector k3v = PVector.mult(a3,SIM_STEP);
  PVector k3s = PVector.mult(v3,SIM_STEP);
  

  // k4v = a(s(t)+k3s, v(t)+k3v)*h
  // k4s = (v(t)+k3v)*h
  PVector v4 = PVector.add(_v,k3v);
  PVector a4 = calculateAcceleration(PVector.add(_v,k3s),v4);
  PVector k4v = PVector.mult(a4,SIM_STEP);
  PVector k4s = PVector.mult(v4,SIM_STEP);
